...
@implementation LWPhotoAblumList
...
- (void)saveImageToAblum:(UIImage *)image completion:(void (^)(BOOL, PHAsset *))completion {
    ...
    if (status == PHAuthorizationStatusDenied) {
        ...
        [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
            ...
            }                                 completionHandler:^(BOOL succ, NSError *_Nullable err) {
                if (completion) completion(succ, asset);
            }];
        }];
    }
}
...
- (NSArray<LWPhotoAblumList *> *)getPhotoAblumList {
    NSMutableArray<LWPhotoAblumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection *_Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (collection.assetCollectionSubtype != PHAssetCollectionSubtypeSmartAlbumVideos
            ...
            if (assets.count > 0) {
                LWPhotoAblumList *ablum = [[LWPhotoAblumList alloc] init];
...
- (void)requestImageForAsset:(PHAsset *)asset size:(CGSize)size synchronous:(BOOL)synchronous completion:(void (^)(UIImage *, NSDictionary *))completion {
    //请求大图界面，当切换图片时，取消上一张图片的请求，对于iCloud端的图片，可以节省流量
    static PHImageRequestID requestID = -1;
    CGFloat scale = [UIScreen mainScreen].scale;
    CGFloat screen_w = [UIScreen mainScreen].bounds.size.width;
    CGFloat width = screen_w > 1000 ? 1000 : screen_w;
...
- (void)requestImageForAsset:(PHAsset *)asset scale:(CGFloat)scale resizeMode:(PHImageRequestOptionsResizeMode)resizeMode completion:(void (^)(UIImage *image))completion {
    ...
    [[PHCachingImageManager defaultManager] requestImageDataForAsset:asset options:option resultHandler:^(NSData *_Nullable imageData, NSString *_Nullable dataUTI, UIImageOrientation orientation, NSDictionary *_Nullable info) {
        BOOL downloadFinined = ![info[PHImageCancelledKey] boolValue] && !info[PHImageErrorKey] && ![info[PHImageResultIsDegradedKey] boolValue];
        if (downloadFinined && completion) {
